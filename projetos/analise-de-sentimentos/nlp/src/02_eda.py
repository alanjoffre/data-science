...
# 7. Distribuição das Pontuações de Sentimento (VADER)
fig, ax = plt.subplots()
sns.histplot(df['sentimento_vader'], bins=20, kde=True, ax=ax)
ax.set_title('Distribuição das Pontuações de Sentimento (VADER)')
save_plot(fig, 'distribuicao_sentimento_vader.png')

# 8. Distribuição das Pontuações de Sentimento (BERT)
fig, ax = plt.subplots()
sns.histplot(df['sentimento_bert'], bins=20, kde=True, ax=ax)
ax.set_title('Distribuição das Pontuações de Sentimento (BERT)')
save_plot(fig, 'distribuicao_sentimento_bert.png')

# 9. Tokens sem Stopwords mais Comuns
fig, ax = plt.subplots()
...
# 11. Tokens Lemmatizados mais Comuns
fig, ax = plt.subplots()
...
# 12. Comparação de Sentimento (VADER vs BERT)
fig, ax = plt.subplots()
sns.scatterplot(x='sentimento_vader', y='sentimento_bert', hue='sentimento', data=df, ax=ax)
ax.set_title('Comparação de Sentimento (VADER vs BERT)')
save_plot(fig, 'comparacao_vader_bert.png')

# 13. Word Clouds para Sentimentos
...
# 15. Correlação Entre Sentimento e Comprimento dos Tweets
fig, ax = plt.subplots()
...
# 16. Frequência de Tokens por Sentimento
...
# 17. Frequência de Bigramas por Sentimento
fig, axes = plt.subplots(nrows=1, ncols=len(sentimentos), figsize=(20, 5))
for ax, sentimento in zip(axes, sentimentos):
