...
def criar_amostra():
    ...
    positivo_df = df[df['sentimento'] == 'positivo'].sample(frac=1).head(600)
    neutro_df = df[df['sentimento'] == 'neutro'].sample(frac=1).head(600)
    negativo_df = df[df['sentimento'] == 'negativo'].sample(frac=1).head(600)
